...
app.get('/host/:id',(req, res)=>{
    async function getByIdHost(){
        try{
            const result = await pool.query(`SELECT * FROM host WHERE host_id = ${req.params.id}`);
...
app.get('/rental/:id',(req, res)=>{
    async function getByIdRental(){
        try{
            const result = await pool.query(`SELECT * FROM rental WHERE rental_id = ${req.params.id}`);
            if(result.rows.length==0){
                res.sendStatus(404);
            }else{
                res.status(200).send(result.rows[0]);
...
app.get('/review/:id',(req, res)=>{
    async function getByIdReview(){
        try{
            const result = await pool.query(`SELECT * FROM review WHERE review_id = ${req.params.id}`);
            if(result.rows.length==0){
                res.sendStatus(404);
            }else{
                res.status(200).send(result.rows[0]);
